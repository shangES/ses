...
AnyChart.useIEOnBeforeUnload = false;
    ...
    if (AnyChart.platform.isIE && AnyChart.platform.isWin) {
        ...
        var dispose = function() {
            if (AnyChart) {
                ...
                if (AnyChart.charts) {
                    len = AnyChart.charts.length - 1;
					for (i = len; i >= 0; i--) {
						if (AnyChart.charts[i])
		...
		if (AnyChart && AnyChart.useIEOnBeforeUnload) {
...
AnyChart.prototype = {
    ...
    _isXMLData: function(data) {
        ...
        while ((strData.charAt(0)=="\n"||strData.charAt(0) == " ") && strData.length > 0) strData = strData.substr(1);
    ...
    showSeries: function(seriesId, isVisible) {
        if (this.flashObject != null && this.flashObject.ShowSeries != null)
            this.flashObject.ShowSeries(seriesId, isVisible);
    },
    ...
    */
	...
	updatePointWithAnimation: function(seriesId, pointId, newValue, animationSettings) {
		if (this.flashObject != null && this.flashObject.updatePointWithAnimation != null)
            this.flashObject.updatePointWithAnimation(seriesId, pointId, newValue, animationSettings);
	},
	
	startPointsAnimation: function() {
		if (this.flashObject != null && this.flashObject.startPointsUpdate)
