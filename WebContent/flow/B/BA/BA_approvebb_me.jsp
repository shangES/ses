...
String page_info=" 【第 "+ now_page_me +" 页/共 "+ total_page +" 页( "+ total_list +" 条)】 ";
...
<cmis:tabpagebar title="待本人处理" url="" onclick="" selected="true" />
<cmis:tabpagebar title="待本环节处理" url="javascript:tous();" onclick="" selected="false" />
...
<table width="100%" border="1" cellspacing="0" cellpadding="1">
	<tr>
		...
		<td class="td1" align="center">时间</td>
		<td class="td1" align="center">借据号</td>
	...
	<% for(int i=listbegin; i<=listend; i++) {
		...
		String process002_1 = process002.substring(0,6);
		String process002_2 = process002.substring(7);
	...
	<tr>
		...
		<%if(!total_page.equals("0")) {%>
			<%if(!now_page_me.equals("1")){%><a 
				...
				src="<%=baseWebPath%>/images/pre_page.gif" width="48" height="24" border="0"></a><%}if(!now_page_me.equals(total_page)){%><a 
...
<input type="hidden" name="approve_returnurl" value="<%=baseWebPath%>/servlet/icbc.cmis.servlets.CmisReqServlet?operationName=icbc.cmis.flow.BA.BA_approvelistOp&opDataUnclear=false&opAction=getmylist&ordercode=<%=ordercode%>&busitype=<%=busitype%>&runproc=<%=runproc%>&now_page_me=<%=now_page_me%>&spflag=<%=spflag%>">
</form>
