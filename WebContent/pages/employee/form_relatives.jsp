...
function remove(){
	...
	if(!confirm('确认要删除选中数据吗？')){
 	...
 	$.post("employee/delHrRelativesById.do",{ids:array.toString()}, function() {
		mygrid.reload();
  	});
}
...
//新增或修改
var addRelativesForm=null;
function addOrEdit(id){
	$("#addRelativesWindow").dialog({
		autoOpen: true,
		modal: true,
		resizable:false,
		width:480,
		buttons: {
			"确定": function() {
				if(addRelativesForm.form){
					$("#addRelativesForm").submit();
				}
			},
			"取消": function() {
				$(this).dialog("close");
			...
			$("#addRelativesForm").clearForm();
			if(id!=null&&id!=''){
				loadRelatives(id);
			}else{
				$("#employeeid").val(tid);
			}
			addRelativesForm=$("#addRelativesForm").validate({submitHandler: function(form) {
			    	$(form).ajaxSubmit(function(data) {
			    		mygrid.reload();
			    		$("#addRelativesWindow").dialog("close");
			        });
				}
			});
		}
...
//得到
function loadRelatives(id){
	if(id!=null&&id!=''&&id!='null')
		$.getJSON("employee/getHrRelativesById.do", {id:id}, function(data) {
			if(data!=null){
				for (var key in data) {
			        var el = $('#' + key);
			        if(el) 
...
function loadGrid(){
	...
	var dsOption= {
		...
		var colsOption = [
			...
			{id: 'employeename' , header: "亲属姓名" , width :80 ,headAlign:'center',align:'left'},
			...
			{id: 'postname' , header: "岗位" , width :120 ,headAlign:'center'},
			{id: 'contactrelationshipname' , header: "两人关系" , width :80 ,headAlign:'center',align:'center'},
		];
		
	var gridOption={
		id : grid_demo_id,
		loadURL :'employee/searchHrRelatives.do',
		beforeLoad:function(reqParam){
			initPagePam();
			reqParam['parameters']=pam;
		},
		exportURL : 'employee/searchHrRelatives.do?export=true',
		beforeExport:function(){
			initPagePam();
			pam.expAll=1;
			mygrid.parameters=pam;
		},
		exportFileName : '员工亲属表.xls',
		width: "99.8%",//"100%", // 700,
		height: "100%",  //"100%", // 330,
		container : 'gridbox', 
		...
		onDblClickCell:function(value, record , cell, row, colNO, rowNO,columnObj,grid){
			if(colNO!=0){
				addOrEdit(record.relativesguid);
			}
		}
	};
...
<!-- 工作经历信息窗口 -->
<div id="addRelativesWindow" title="公司亲属信息" style="display:none;">
	<form action="employee/saveOrUpdateRelatives.do" id="addRelativesForm" class="form" method="post">
		<input type="hidden" id="relativesguid" name="relativesguid" value=""/>
		<input type="hidden" id="employeeid" name="employeeid" value=""/>
		<ul>
			<li>
			    <span><em class="red">* </em>两人关系：</span>
		...
		<ul>
			<li>
			    ...
			    <input id="t_employeeid" name="t_employeeid" type="hidden" value=""/>
			    <input id="employeename" name="employeename" class="{required:true} inputselectstyle" onclick="chooseinterviewerTree('#t_employeeid','#employeename','',callbackUser);"/>
			    <div class="select-trigger" onclick="chooseinterviewerTree('#t_employeeid','#employeename','',callbackUser);"/>
			</li>
		</ul>
		<ul>
			<li>
				<span><em class="red">* </em>公司：</span>
				<input  id="companyname" name="companyname"  class="{required:true,maxlength:50} inputstyle disabled" readOnly="true"/>
			</li>
		</ul>
		<ul>
			<li>
				<span><em class="red">* </em>部门：</span>
				<input  id="deptname" name="deptname"  class="{required:true,maxlength:50} inputstyle disabled" readOnly="true"/>
			</li>
		</ul>
		<ul>
			<li>
				<span><em class="red">* </em>岗位：</span>
				<input id="postname" name="postname" class="{required:true,maxlength:50} inputstyle disabled" readOnly="true"/>
			</li>
		</ul>
	</form>
</div>
