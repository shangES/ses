...
function validCarousel(valid){
	...
	if(!confirm("确认要"+prompt+"选中数据吗？")){
   	...
   	$.post("recruitment/validCarouselById.do",{ids:array.toString(),state:valid}, function() {
		mygrid.reload();
    });
}
...
function remove(){
	...
	var cords=mygrid.getSelectedRecords();
	for(var i=0;i<cords.length;i++){
		array.push(cords[i].carouselguid);
	}
	if(array.length<=0){
		alert('请选择要删除的数据！');
		return;
	...
	$.post("recruitment/delCarouselById.do",{ids:array.toString()}, function() {
		mygrid.reload();
    });
}
...
//失效、还原
function validOneCarousel(id,valid){
	var prompt=(valid==0?"失效":"恢复");
	var array=new Array();
	array.push(id);
	if(!confirm("确认要"+prompt+"选中数据吗？")){
...
function audit(valid){
	...
	if(!confirm("确认要"+prompt+"选中数据吗？")){
   	...
   	$.post("recruitment/auditCarouselById.do",{ids:array.toString(),state:valid}, function() {
		mygrid.reload();
    });
}
	...
	var dsOption= {
		...
		var colsOption = [
			...
			{id: 'valid' , header: "操作" , width :80 ,headAlign:'center',align:'center',hidden:true,renderer:function(value ,record,colObj,grid,colNo,rowNo){
				var htm='';
        			htm+='<a href="javascript:convertView(\'carousel.do?page=form&id='+record.carouselguid+'\');" class="ui-button ui-button-icon-only" title="修改"><span class="ui-icon ui-icon-pencil"></span>&nbsp;</a>';
       			if(value==1)
	        		htm+='<a href="javascript:validOneCarousel(\''+record.carouselguid+'\',0);" class="ui-button ui-button-icon-only" title="置为无效"><span class="ui-icon ui-icon-trash"></span>&nbsp;</a>';
	        	else if(value==0)
	        		htm+='<a href="javascript:validOneCarousel(\''+record.carouselguid+'\',1);" class="ui-button ui-button-icon-only" title="还原"><span class="ui-icon ui-icon-check"></span>&nbsp;</a>';
				return htm;
			}},
			...
			{id: 'title' , header: "标题" , width :300  ,headAlign:'center'},
			{id: 'pubuser' , header: "发布人" , width :110 ,headAlign:'center'},
			{id: 'modtime' , header: "发布时间" , width :150,headAlign:'center',align:'center'},
			{id: 'audituser' , header: "审核人" , width :110,headAlign:'center'},
			{id: 'rmk' , header: "备注" , width :200 ,headAlign:'center',align:'left'}
		];
		
	var gridOption={
		id : grid_demo_id,
		loadURL :'recruitment/searchCarousel.do',
		beforeLoad:function(reqParam){
			initPagePam();
			reqParam['parameters']=pam;
		},
		exportURL : 'recruitment/searchCarousel.do?export=true',
		beforeExport:function(){
			initPagePam();
			pam.expAll=1;
			mygrid.parameters=pam;
		},
		exportFileName : '轮播图片表.xls',
		width: "99.8%",//"100%", // 700,
		height: "100%",  //"100%", // 330,
		container : 'gridbox', 
		...
		onDblClickCell:function(value, record , cell, row, colNO, rowNO,columnObj,grid){
			if(colNO!=0){
				convertView('carousel.do?page=form&id='+record.carouselguid);
			}
		}
	};
		...
		<div class="table">
			<div class="table-bar">
				...
				<div class="table-ctrl">
					...
					<a class="btn" href="javascript:convertView('carousel.do?page=form');"  style="display:${add?'':'none'}"><i class="icon icon-plus"></i><span>新增</span></a>
					...
					<!-- 
					<a class="btn"  href="javascript:validCarousel(0);" style="display:${valid?'':'none'}" ><i class="icon icon-trash"></i><span>失效</span></a>
		   			<a class="btn"  href="javascript:validCarousel(1);" style="display:${revert?'':'none'}" ><i class="icon icon-retweet"></i><span>恢复</span></a>
		   			 -->
