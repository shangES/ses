...
strFrame='<style>'; 
strFrame+='INPUT.button{BORDER-RIGHT: #C0C0C0 1px solid;BORDER-TOP: #C0C0C0 1px solid;BORDER-LEFT: #C0C0C0 1px solid;'; 
strFrame+='BORDER-BOTTOM: #C0C0C0 1px solid;BACKGROUND-COLOR: #ffffff;font-family:宋体;}'; 
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 width=142 height=160 bordercolor=#C0C0C0 bgcolor=#C0C0C0 Author="wayx">'; 
...
strFrame+=' <tr align=center Author="wayx"><td width=16 align=center bgcolor=#C0C0C0 style="font-size:12px;cursor: hand;color: #ffffff" '; 
...
strFrame+=' <td width=16 bgcolor=#C0C0C0 align=center style="font-size:12px;cursor: hand;color: #ffffff" '; 
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 bgcolor=#C0C0C0 ' + (bMoveable? 'onmousedown="DragStart()" onmouseup="DragEnd()"':''); 
strFrame+=' BORDERCOLORLIGHT=#C0C0C0 BORDERCOLORDARK=#FFFFFF width=140 height=20 Author="wayx" style="cursor:' + (bMoveable ? 'move':'default') + '">'; 
...
strFrame+=' <table border=1 cellspacing=2 cellpadding=0 BORDERCOLORLIGHT=#C0C0C0 BORDERCOLORDARK=#FFFFFF bgcolor=#ffffff width=140 height=120 Author="wayx">'; 
...
strFrame+=' onfocus="this.blur()" title="当前浏览器日期" style="font-size: 12px; height: 20px; cursor:hand"></td><td '; 
...
{ 
	...
	if(r!=null){ 
		...
		//zyz增加校验，如果输入格式错误，就不解析
		if(r[1].length<4 || r[2].length<2 || r[3].length<2 ){
			outDate=""; 
...
//meizzSetDay(meizzTheYear,meizzTheMonth); 
if(outObject){ 
if (meizzTheMonth < 10){meizzTheMonth = "0" + meizzTheMonth;}if (today < 10){today = "0" + today;} 
//今天的输出格式
