...
function chooseContractID(){
   if(isEmpty(form1.Apply_customerCode.value)){
        ...
        return;
        	...
        	var ts = window.showModalDialog(basepath0606+"/servlet/icbc.cmis.servlets.CmisReqServlet?operationName=icbc.cmis.TD.TDPackLoanApplyOp&opDataUnclear=true&ta200061001="+form1.Apply_customerCode.value+"&area="+form1.Apply_sponsorBank.value+"&oper=query&time="+new Date(),window,"dialogWidth:500px;dialogHeight:320px;center:yes;help:no;status:no;resizable:no");
        	if(ts!=null){
