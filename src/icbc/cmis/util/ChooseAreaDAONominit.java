...
public class ChooseAreaDAONominit extends CmisDao {

  public ChooseAreaDAONominit(icbc.cmis.operation.CmisOperation op) {
  super(op);
  }
...
//    sql = "select AREA_CODE, AREA_NAME, BANK_FLAG, BELONG_BANK from mag_area  start with area_code = '" + areaCode+"' connect by area_code = prior belong_bank union select AREA_CODE, AREA_NAME, BANK_FLAG, BELONG_BANK from mag_area,minit  where belong_bank = '"+ areaCode +"' and BANK_FLAG < " + disable + " and area_code = local_code(+) and local_code is not null ";
      pstmt.close();
      sql = "select AREA_CODE, AREA_NAME, BANK_FLAG, BELONG_BANK from mag_area  start with area_code = ? connect by area_code = prior belong_bank union select AREA_CODE, AREA_NAME, BANK_FLAG, BELONG_BANK from mag_area  where belong_bank = ? and BANK_FLAG < ?";   // and area_code = local_code(+) and local_code is not null ";
      pstmt = conn.prepareStatement(sql);
      pstmt.setString(1, areaCode);
...
//      rs = stmt.executeQuery(sql);
      ...
      throw new TranFailException("cmisutil212","icbc.cmis.util.ChooseAreaDAONominit",ex.getMessage() + sql,"产生地区列表错误！");
    }
    finally {
