...
//        newString += "ROWNUM <= "+ bufferEnd +" ) WHERE rnum >= "+ bufferBegin ;
        ...
        try {
          ...
          throw new TranFailException("query","icbc.cmis.util.TableAdjustView",ex.getMessage(),"取缓存数据出错！");
        }
      }
      ...
      throw new TranFailException("query","icbc.cmis.util.TableAdjustView",ex.getMessage(),"取数据出错！");
    }
