...
public class ChooseBelongBankDAO extends CmisDao {

  public ChooseBelongBankDAO(icbc.cmis.operation.CmisOperation op) {
  super(op);
  }
...
//      sql = "select AREA_CODE, AREA_NAME, BANK_FLAG from mag_area where AREA_CODE = '" + areaCode + "'";
     //判断是否是总行
     if (areaCode.equals("00000000")){
        sql="select AREA_CODE, AREA_NAME, BANK_FLAG,AREA_CODE from mag_area where AREA_CODE =?";
     }
     else{
      sql = "select AREA_CODE, AREA_NAME, BANK_FLAG,BELONG_BANK from mag_area where AREA_CODE =?";
     }
      ...
      String belongbank=rs.getString(4);
      //查询上级行及下一级行
//    sql = "select AREA_CODE, AREA_NAME, BANK_FLAG, BELONG_BANK from mag_area  start with area_code = '" + areaCode+"' connect by area_code = prior belong_bank union select AREA_CODE, AREA_NAME, BANK_FLAG, BELONG_BANK from mag_area,minit  where belong_bank = '"+ areaCode +"' and BANK_FLAG < " + disable + " and area_code = local_code(+) and local_code is not null ";
      pstmt.close();
      sql = "select AREA_CODE, AREA_NAME, BANK_FLAG, BELONG_BANK from mag_area  start with area_code = ? connect by prior belong_bank = area_code  union select AREA_CODE, AREA_NAME, BANK_FLAG, BELONG_BANK from mag_area,minit  where BANK_FLAG >= ? and area_code = local_code(+) and local_code is not null ";
      pstmt = conn.prepareStatement(sql);
      //pstmt.setString(1, belongbank);
      pstmt.setString(1, areaCode);
      pstmt.setInt(2,disable);
...
//      rs = stmt.executeQuery(sql);
      ...
      throw new TranFailException("cmisutil212","icbc.cmis.util.ChooseBelongBankDAO",ex.getMessage() + sql,"产生地区列表错误！");
    }
    finally {
