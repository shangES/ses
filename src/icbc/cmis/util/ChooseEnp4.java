...
public class ChooseEnp4 extends HttpServlet {
...
//  private static final String CONTENT_TYPE = "text/xml";
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws javax.servlet.ServletException, java.io.IOException {
    ...
    try {
      if(!CMisSessionMgr.checkSession(request)){
        outTime(out,"  当前会话已失效，请重新登录<br>Please log on again");
        return;
      }
  ...
  public void query(CMisSessionMgr session,PrintWriter out,String spage) throws TranFailException{
	...
	propertyResourceReader = new icbc.cmis.tags.PropertyResourceReader(langCode,"icbc.cmis.util.util_comm_EnpChoose"); 
    try {
      //从SESSION中取回数据
...
//updated by chenj 20030526
        ...
        try {
          ...
          throw new TranFailException("cmisUTIL803","icbc.cmis.util.ChooseEnp4",ex.getMessage(),genMsg.getErrMsgByLang(langCode,"099993"));
        }
      }
      ...
      if(datas != null && !datas.isEmpty()) {
        for(int i = 0; i <= end - begin; i++) {
		  ...
		  // lxm 20060220 增加客户状态
		  if(row[4].equals("1")){
			row[4]= propertyResourceReader.getPrivateStr("C000031");  //"已建立信贷关系";
		  }else if(row[4].equals("2")){
			row[4]= propertyResourceReader.getPrivateStr("C000032");  //"未建立信贷关系";
		  }else if(row[4].equals("3")){
			row[4]= propertyResourceReader.getPrivateStr("C000033");  //"销户(无业务)";
		  }else if(row[4].equals("4")){
			row[4]= propertyResourceReader.getPrivateStr("C000034");  //"被整合";
		  }else if(row[4].equals("5")){
		    row[4]= propertyResourceReader.getPrivateStr("C000035");  //"销户(已结清)";
		  }
          out.print("<en o='" + row[5] + "' c='" + row[0] + "' n='" + row[1] + "' j='" + row[2] + "' d='" + row[3] + "' z='" + row[4] + "' />");
        }
      }
      out.print("</datas>");
    } catch (TranFailException ex) {
      ...
      throw new TranFailException("cmisUTIL801","icbc.cmis.util.ChooseEnp4",ex.getMessage(),genMsg.getErrMsgByLang(langCode,"099993"));
    }
