...
public class gene_qianpiOp extends CmisOperation {
	...
	public void execute() throws java.lang.Exception, TranFailException {
		try {
			...
			if (opAction.equals("10000")) { //校验
				tocheck();
			} else if (opAction.equals("10001")) { //签批处理
				toqianpi();
			}
	...
	public void toqianpi() throws Exception {
		try {
			...
			gene_qianpiDao dao = new gene_qianpiDao(this);
			HashMap hmaprst = dao.doqianpi(hmapinfo);
			...
			throw new TranFailException("icbc.cmis.flow.BTN", "gene_qianpiOp.toqianpi()", e.getMessage(), e.getMessage());
		}
	}
