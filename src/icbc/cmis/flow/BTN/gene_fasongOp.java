...
public class gene_fasongOp extends CmisOperation {
	...
	public void execute() throws java.lang.Exception, TranFailException {
		try {
			...
			if (opAction.equals("10000")) { //校验
				tocheck();
			} else if (opAction.equals("10001")) { //发送处理
				tofasong();
			}
	...
	public void tofasong() throws Exception {
		try {
			...
			gene_fasongDao dao = new gene_fasongDao(this);
			HashMap hmaprst = dao.dofasong(hmapinfo);
			...
			throw new TranFailException("icbc.cmis.flow.BTN", "gene_fasongOp.tofasong()", e.getMessage(), e.getMessage());
		}
	}
