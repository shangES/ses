...
public class gene_tuihuiOp extends CmisOperation {
	...
	public void execute() throws java.lang.Exception, TranFailException {
		try {
			...
			if (opAction.equals("10000")) { //校验
				tocheck();
			} else if (opAction.equals("10001")) { //退回处理
				totuihui();
			}
	...
	public void totuihui() throws Exception {
		try {
			...
			tuihuiDao daoPub = new tuihuiDao(this);
			String approve_backnum = daoPub.querybackemp(this.getStringAt("approve_entcode"), this.getStringAt("approve_tradecode"));

			gene_tuihuiDao dao = new gene_tuihuiDao(this);
			HashMap hmaprst = dao.dotuihui(hmapinfo, approve_backnum);
			...
			throw new TranFailException("icbc.cmis.flow.BTN", "gene_tuihuiOp.totuihui()", e.getMessage(), e.getMessage());
		}
	}
