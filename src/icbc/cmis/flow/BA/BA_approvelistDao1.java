...
public class BA_approvelistDao1 extends CmisDao {

	String sLangCode = "zh_CN";

	public BA_approvelistDao1(icbc.cmis.operation.CmisOperation op) {
		super(op);
		try {
			sLangCode = (String) op.getSessionData("LangCode");
		} catch (Exception exp) {
		}
	}
	...
	public ArrayList querylist_me(String employeecode, String empareacode,
		...
		try {
			...
			sql = "select rownum rnum,process001,ta200011003 process001_name,process002,"
					...
					+ "from mprocess_new, ta200011, imag_kind, pa200021 "
					+ "where ta200011001 = process001 and kind_code = process003 and pa001 = process004 "
					+ "and process004 in(select pa001 from pa200021 where pa004 = ?  and lang_code='"+sLangCode+"') "
					+ "and process006 = ? and process007 = ? and process008 = ? and ( process009 = '0' or process009 = '1' ) and pa200021.lang_code='"+sLangCode+"' and imag_kind.LANG_CODE=? "
					+ " union all "
					+ "select rownum rnum,process001,ta340001002 process001_name,process002,"
					+ "process003,kind_name process003_name, "
					+ "process004,pa002 process004_name, "
					+ "process005,process009 "
					+ "from mprocess_new, ta340001, imag_kind, pa200021 "
					+ "where ta340001001 = process001 and kind_code = process003 and pa001 = process004 "
					+ "and process004 in (select pa001 from pa200021 where pa004 = ?  and lang_code='"+sLangCode+"') "
					+ "and process006 = ? and process007 = ? and process008 = ? and ( process009 = '0' or process009 = '1' ) and pa200021.lang_code='"+sLangCode+"' and imag_kind.LANG_CODE=? ";
			...
			pstmt.setString(6, busitype);
			pstmt.setString(7, ordercode);
			pstmt.setString(8, empareacode);
			pstmt.setString(9, employeecode);
			pstmt.setString(10, this.sLangCode);
	...
	public ArrayList querylist_us(String empareacode, String employeecode,
		...
		try {
			...
			sql = "select rownum rnum,process001,ta200011003 process001_name,process002,"
					...
					+ "where ta200011001 = process001 and kind_code = process003 and pa001 = process004 "
					+ "and process004 in(select pa001 from pa200021 where pa004 = ? and lang_code='"+sLangCode+"' ) "
					+ "and process006 = ? and process007 = ? and (process008 is null or process008='') and process009 = '0' and pa200021.lang_code='"+sLangCode+"' and imag_kind.LANG_CODE=? "
					+ " union all "
					+ "select rownum rnum,process001,ta340001002 process001_name,process002,"
					+ "process003,kind_name process003_name, "
					+ "process004,pa002 process004_name, "
			...
			pstmt.setString(5, busitype);
			pstmt.setString(6, ordercode);
			pstmt.setString(7, empareacode);
			pstmt.setString(8, this.sLangCode);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				HashMap hmap = new HashMap();
				hmap.put("rnum", killnull(rs.getString("rnum")));
			...
			if (runproc.equals("1")) {
				...
				sql = "select interface001 from mag_approve_interface "
						+ "where tradecode in(select pa001 from pa200021 where pa004 = ? and lang_code='"+sLangCode+"' ) "
