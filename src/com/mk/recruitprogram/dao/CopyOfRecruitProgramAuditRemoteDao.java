...
public class CopyOfRecruitProgramAuditRemoteDao extends JdbcDaoSupport {
	...
	public List<RecruitProgramAudit> getRemoteRecruitProgramAuditRemote() {
		...
		sql.append("select t1.SQRGS,t1.ID, t1.DEPATRMENT1 ,t1.DEPATRMENT2 ,t1.GW,t1.sqdate,t1.qwdgsj, t1.SQRS, t1.ZJ1,t1.SQRBH, t2.REQUESTID from ecology.formtable_main_163 t1, ecology.workflow_requestbase t2 where t1.requestid=t2.requestid and t2.currentnodetype=3");
		...
		this.getJdbcTemplate().query(sql.toString(), new RowMapper<RecruitProgramAudit>() {
			public RecruitProgramAudit mapRow(ResultSet rst, int rowNumber) throws SQLException {
				while (rst.next()) {
					...
					//得到申请公司
					if(StringUtils.isNotEmpty(rst.getString("SQRGS"))){
						model.setCompanyname(rst.getString("SQRGS").replaceAll(" ", ""));
					}
