...
@Service
public class RecruitProgramAuditService {
	@Autowired
			...
			for(RecruitProgramAudit model : list){
				...
				if(recruit == null){
					if(model != null && StringUtils.isNotEmpty(model.getDeptname())){
						Department department = departmentdao.getDepartmentbyDepartmentName(model.getDeptname(),Constance.COMPANYID);
							...
							if(position != null){
								...
								if(dept!=null){
									if(StringUtils.isEmpty(dept.getPdeptid())){
										...
										List<Department> pdepts = departmentdao.getDepartmentByPDeptid(Constance.COMPANYID, dept.getPdeptid());
						...
						if(StringUtils.isNotEmpty(model.getRankname())){
							Rank rank = companyDao.getRankByInterfacecode(model.getRankname());
							if(rank!=null){
